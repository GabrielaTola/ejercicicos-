...
class TicTacToe(Juego):
    ...
    def __init__(self, h=3, v=3, k=3, *args, **kwargs):
        ...
        self.inicial = EstadoJuego(mover_a='X', utilidad=0, tablero={}, movimientos=movimientos)
    ...
    def resultado(self, estado, jugador_actual_jugada_ejecutada):
        ...
        return EstadoJuego(mover_a=('O' if estado.mover_a == 'X' else 'X'),
...
class JugadorAleatorio(Jugador):
    ...
    def obtener_accion(self, estado, turno=1, verbose=0):
        ...
        return random.choice(self.juego.acciones(estado))
...
class JugadorMiniMax(Jugador):
    CONTAR = 0
    def obtener_accion(self, estado, turno=1, verbose=0):
        return minimax_decision(estado, self.juego)
